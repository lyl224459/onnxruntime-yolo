...
The default CUDA installer path uses a reduced component list based on NVIDIA's Windows silent installer sub-package names for faster CI setup. If that component install fails, the script automatically retries with the full CUDA silent installer. A custom component list can still be forced through `ORT_YOLO_CUDA_INSTALL_COMPONENTS`.
